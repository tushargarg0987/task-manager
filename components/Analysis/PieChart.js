...
function PlottedPieChart() {
    ...
    const sliceColor = ['#EB455F','#537FE7','#FFEB3B']
    const arr = [];
    for (let ele in listValues) {
        let count=0;
        for (let ele2 in expensesCtx.expenses) {
            if (listValues[ele].value === expensesCtx.expenses[ele2].priority) {
                count = count + 1;
            }
        }
        total = total + count;
    ...
    return (
        ...
        <Text style={{fontSize: 18, fontWeight: '500'}}>Based on Priority</Text>
        <PieChart
            data={arr}
                ...
                {arr.map((component) => {
                    return (
                        <View key={component.key} style={{flexDirection: "row",marginVertical: 8}}>
                            ...
                            <Text>  {((component.expense / total) * 100).toFixed(1)}% are { component.name.charAt(0).toUpperCase() + component.name.slice(1)}</Text>
                        </View>
                    )
                })}
            </View>
